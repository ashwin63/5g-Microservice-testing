...
class Request(object):
    ...
    @staticmethod
    def init_value_generators_from_corpus(fuzzable_request_blocks, fuzzable, value_gen_tracker, hex_definition):
        value_generators = {}
        filename = os.path.join(logger.DYNAMIC_VALUES_DIR,hex_definition)
        with open(filename, 'r') as file:
            data = json.load(file)
        print(data[0][str(idx)])
        for idx in fuzzable_request_blocks:
            if fuzzable[idx] \
            and isinstance(fuzzable[idx][0], tuple)\
            and primitives.is_value_generator(fuzzable[idx][0][0]):
    ...
    def render_iter(self, candidate_values_pool, skip=0, preprocessing=False, prev_rendered_values=None,
        ...
        for (req, is_example) in schema_combinations:
            ...
            if schema_idx not in self._rendered_values_cache._value_generators:
                ...
                value_generators = Request.init_value_generators_from_corpus(fuzzable_request_blocks, fuzzable,
                                                                 value_gen_tracker, hex_definition)
